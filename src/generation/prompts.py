...
2. **[중요]** 답변 마지막 줄에 반드시 출처를 표기하세요. 형식은 정확히 다음과 같아야 합니다:
   `[출처: doc_id, 카테고리, 캠퍼스]`
...
def format_context(candidates: Sequence[dict[str, Any]]) -> str:
    ...
    for i, c in enumerate(candidates, start=1):
        ...
        meta_bits = [
            ...
            f"카테고리={payload.get('category')}",
            f"하위={payload.get('subcategory')}",
            f"캠퍼스={payload.get('campus')}",
            f"출처={payload.get('source_collection')}",
        ]
        meta = " | ".join(b for b in meta_bits if b and "=None" not in b)
